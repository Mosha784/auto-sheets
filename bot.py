...
def col(*parts):
    for i, h in enumerate(headers):
        if all(p in h for p in parts): return i
    return -1
...
C = {"img": col("product link"), "ali_price": col("alibaba", "price"),
     "cn_price": col("1688", "price"), "L": col("length"), "W": col("width"),
     "H": col("height"), "WT": col("weight"), "items": col("items per carton"),
     "cbm": col("cbm"), "ali_link": col("alibaba", "link"),
     "ali_photo": col("alibaba", "photo"), "cn_link": col("1688", "link"),
     "cn_photo": col("1688", "product"), "land_ali": col("alibaba", "landed"),
     "land_cn": col("1688", "landed")}

try: meta = sh.worksheet("meta")
except gspread.WorksheetNotFound: meta = sh.add_worksheet("meta", rows=5, cols=2)

def progress():
    v = meta.acell("A1").value
    return int(v) if v and v.isdigit() else 0

def set_progress(n): meta.update_acell("A1", str(n))
...
def extract(d, lk):
    def find(subs):
        ...
        return None
    title = find(("subject","title","name")); url = find(("detailurl","producturl","offerurl","url","link","href")); price = find(("price",))
...
def make_hook(bucket):
    def f(resp):
        try:
            if resp.status != 200 or "json" not in (resp.headers.get("content-type") or ""): return
...
def upload(page, img_path, sels):
    try:
        ...
        if inp: inp.set_input_files(img_path); return True
    except Exception: pass
    for s in sels:
        try:
            with page.expect_file_chooser(timeout=4000) as fc: page.click(s, timeout=4000)
            fc.value.set_files(img_path); return True
...
def image_search(ctx, img_path, site):
    page = ctx.new_page(); bucket = []
    page.on("response", make_hook(bucket)); cands = []
    try:
        if site == "alibaba":
            ...
            if "login" in page.url or "passport" in page.url:
                print("1688: محتاج تسجيل دخول — متخطي"); page.close(); return []
    ...
    page.close(); return cands[:5]

# ---------- سحب صفحة المنتج ----------
def scrape_alibaba(ctx, url):
    info = {}; page = ctx.new_page()
    try:
        ...
        if m: info["dims"] = [float(m.group(i)) for i in (1, 2, 3)]
        m = re.search(r"Single gross weight:?\s*([\d.,]+)\s*kg", t, re.I)
        if m: info["weight"] = float(m.group(1).replace(",", ""))
        prices = [float(p.replace(",", "")) for p in re.findall(r"(?:US\s*)?\$\s*([\d,]+(?:\.\d+)?)", t)]
        prices = [p for p in prices if 0.05 <= p <= 100000]
        if prices: info["price"] = min(prices)
    except Exception as e: print("alibaba page err:", str(e)[:100])
    ...
    try:
        ...
        for kw in ["包装尺寸", "外箱尺寸", "单件尺寸", "尺寸"]:
            ...
            if i >= 0:
                ...
                if m: info["dims"] = [float(m.group(k)) for k in (1, 2, 3)]; break
    except Exception as e: print("1688 page err:", str(e)[:100])
    page.close(); return info

# ---------- الكتابة ----------
def put(updates, r, c, v):
    if c >= 0: updates.append((r, c, v))

def flush(updates):
    if not updates: return
    data = [{"range": f"'{ws.title}'!{cl(c)}{r}", "values": [[v]]} for r, c, v in updates]
...
def process(i, img, updates, ctx):
    ...
    if not download(img, path):
        print("تحميل الصورة فشل"); return
    dims = None; ali_aed = None; cn_aed = None
    ...
    if ali:
        ...
        if info.get("price"):
            ali_aed = round(info["price"] * USD_AED, 2); put(updates, r, C["ali_price"], ali_aed)
        if info.get("dims"):
            dims = info["dims"]
            put(updates, r, C["L"], dims[0]); put(updates, r, C["W"], dims[1]); put(updates, r, C["H"], dims[2])
        if info.get("weight"): put(updates, r, C["WT"], info["weight"])
    ...
    if cn:
        ...
        if info.get("price"):
            cn_aed = round(info["price"] * CNY_AED, 2); put(updates, r, C["cn_price"], cn_aed)
    ...
    if L and W and H and cbm_cost:
        ...
        if ali_aed: put(updates, r, C["land_ali"], round(ali_aed + ship, 2))
        if cn_aed: put(updates, r, C["land_cn"], round(cn_aed + ship, 2))
...
def main():
    ...
    with sync_playwright() as p:
        ...
        while i < total and time.time() - t0 < BUDGET:
            ...
            if img:
                try: process(i, img, updates, ctx)
                except Exception as e: print("row err:", str(e)[:100])
            i += 1; set_progress(i)
            time.sleep(random.uniform(2, 4))
        ctx.close()
    flush(updates)
    print("خلصنا صف", i, "من", total)
